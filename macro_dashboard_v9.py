...
def load_econ_calendar(api_key):
    """
    Fetch CONFIRMED upcoming economic release dates directly from the FRED API.
    Uses /fred/releases/dates endpoint — these are the actual scheduled dates
    published by FRED, BLS, BEA, Fed, etc. No guesswork or approximation.

    Returns a list of dicts: {name, date, days_away, impact, source}
    sorted by date ascending, next 14 days only.
    """
    import requests as _req

    # FRED release IDs that correspond to high-market-impact reports
    # (release_id, display_name, impact_level, source_agency)
    RELEASES = [
    ...
    today      = date.today()
    look_ahead = today + timedelta(days=60)   # fetch up to 60 days ahead
    results    = []
    ...
    for release_id, name, impact, source in RELEASES:
        try:
            ...
            # Each entry: {"release_id": ..., "date": "YYYY-MM-DD"}
            for entry in dates_raw:
                ...
                break  # take the next upcoming date only per release
        except Exception:
            continue

    # Sort by date, deduplicate by name (keep earliest)
    seen = set()
    unique = []
    ...
    return unique[:14]  # return up to 14 upcoming events
...
desc("All dates are <strong>confirmed and sourced directly from the FRED API</strong> (Federal Reserve Economic Data), "
     ...
     "This calendar auto-updates every 6 hours — no guesswork or approximate dates.")
...
if econ_cal:
    ...
    st.caption(f"Source: FRED API (api.stlouisfed.org) · Dates are official confirmed schedule · "
               f"Refreshed every 6 hours · Last fetch: {now_sgt().strftime('%d %b %Y %H:%M')} SGT")
else:
    st.info(
        "Economic calendar is loading or temporarily unavailable. "
        "The calendar pulls confirmed dates from the FRED API. "
