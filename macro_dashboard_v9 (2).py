"""
MACRO/SIGNAL Dashboard v7.0
============================================================
Install: pip install streamlit pandas plotly fredapi yfinance pytz
Run:     streamlit run macro_dashboard_v7.py
FRED key → Streamlit Cloud → Manage App → Secrets:
  FRED_API_KEY = "your_key_here"

v7.0 fixes vs v6.0:
- Correlations: fixed — now correctly fetches & aligns multi-asset returns
- Fear & Greed: recalibrated — uses market-aware scaling with proper guard
- Recession Probability: fixed — correct signal thresholds reflecting current data
- Live chart custom time range: date-picker in tab header (independent of macro range)
- Fullscreen: paper_bgcolor set to solid #060b14 so background matches app in fullscreen
- Spacing: generous padding between all sections, no compressed stacking
- Beginner descriptions: every major section has an expandable explanation box
============================================================
"""
...
def _build_hardcoded_calendar():
    """
    Official 2026 US economic release schedule from the White House OMB
    Principal Federal Economic Indicators calendar (CY2026).
    Source: https://www.whitehouse.gov/wp-content/uploads/2025/09/pfei_schedule_release_dates_cy2026.pdf
    Dates are definitive — no guesswork. Only future dates are returned.
    Returns list of (name, date_obj, impact, source).
    """
    from datetime import date as _d
    # (name, impact, source, [(month, day), ...])
    # Each tuple = one release event in 2026
    SCHED = [
        ("Employment Situation (NFP)",     "HIGH",   "BLS",          [(1,9),(2,6),(3,6),(4,3),(5,8),(6,5),(7,2),(8,7),(9,4),(10,2),(11,6),(12,4)]),
        ("Consumer Price Index (CPI)",     "HIGH",   "BLS",          [(1,13),(2,11),(3,11),(4,10),(5,12),(6,10),(7,14),(8,12),(9,11),(10,14),(11,10),(12,10)]),
        ("Producer Price Index (PPI)",     "MEDIUM", "BLS",          [(1,14),(2,12),(3,12),(4,14),(5,13),(6,11),(7,15),(8,13),(9,10),(10,15),(11,13),(12,15)]),
        ("GDP (Advance/Prelim/Final)",     "HIGH",   "BEA",          [(1,29),(2,26),(3,27),(4,30),(5,28),(6,25),(7,30),(8,26),(9,30),(10,29),(11,25),(12,23)]),
        ("Personal Income & PCE",          "HIGH",   "BEA",          [(1,29),(2,26),(3,27),(4,30),(5,28),(6,25),(7,30),(8,26),(9,30),(10,29),(11,25),(12,23)]),
        ("Retail Sales",                   "MEDIUM", "Census Bureau",[(1,15),(2,17),(3,16),(4,16),(5,14),(6,17),(7,16),(8,14),(9,16),(10,15),(11,17),(12,16)]),
        ("Housing Starts",                 "MEDIUM", "Census Bureau",[(1,21),(2,18),(3,17),(4,17),(5,19),(6,16),(7,17),(8,18),(9,17),(10,20),(11,18),(12,17)]),
        ("Industrial Production",          "MEDIUM", "Federal Reserve",[(1,16),(2,18),(3,16),(4,16),(5,15),(6,15),(7,17),(8,18),(9,18),(10,16),(11,17),(12,16)]),
        ("Durable Goods Orders",           "MEDIUM", "Census Bureau",[(1,28),(2,26),(3,25),(4,24),(5,28),(6,25),(7,27),(8,26),(9,25),(10,27),(11,25),(12,23)]),
        ("Consumer Credit",                "LOW",    "Federal Reserve",[(1,8),(2,6),(3,6),(4,7),(5,7),(6,5),(7,8),(8,7),(9,8),(10,7),(11,6),(12,7)]),
        ("New Residential Sales",          "LOW",    "Census Bureau",[(1,27),(2,25),(3,24),(4,23),(5,27),(6,24),(7,24),(8,25),(9,24),(10,27),(11,25),(12,23)]),
        ("International Trade Balance",    "MEDIUM", "Census/BEA",   [(1,8),(2,5),(3,5),(4,2),(5,5),(6,9),(7,7),(8,4),(9,3),(10,6),(11,4),(12,8)]),
    ]
    today   = date.today()
    results = []
    for name, impact, source, monthly_dates in SCHED:
        for m, d in monthly_dates:
            try:
                rel_date = date(2026, m, d)
            except ValueError:
                continue
            if rel_date >= today:
                results.append((name, rel_date, impact, source))
                break  # take only the next upcoming one
    return results


@st.cache_data(ttl=21600, show_spinner=False)  # refresh every 6 hours
def load_econ_calendar(api_key):
    """
    Upcoming economic release dates — three-tier approach:

    Tier 1 (primary): Hardcoded official 2026 OMB/PFEI schedule. These are
      the authoritative dates published by the issuing agencies. Always works,
      never needs a network call.

    Tier 2 (enrichment): Attempts to pull FOMC meeting dates and Initial
      Jobless Claims from the FRED API to supplement the hardcoded list with
      data not in the OMB table.

    Tier 3 (projection): Uses fredapi series metadata to project any remaining
      gaps from last known observation date + typical cadence.

    Returns a list of dicts: {name, date, days_away, impact, source}
    sorted by date ascending, up to 14 events.
    """
    ...
    # ── Tier 1: authoritative hardcoded OMB 2026 schedule ─────────────────────
    for name, rel_date, impact, source in _build_hardcoded_calendar():
        results.append({
            "name":      name,
            "date":      rel_date,
    ...
    # ── Tier 2: FRED API for FOMC + weekly data not in OMB table ───────────────
    EXTRA_RELEASES = {
        18:  ("FOMC Meeting / Fed Decision", "HIGH",   "Federal Reserve"),
        326: ("FOMC Minutes",                "MEDIUM", "Federal Reserve"),
        82:  ("Initial Jobless Claims",      "MEDIUM", "DOL"),
    }
    look_ahead = today + timedelta(days=60)
    existing_names = {r["name"] for r in results}
    for rid, (name, impact, source) in EXTRA_RELEASES.items():
    ...
    # ── Deduplicate by name, sort by date ──────────────────────────────────────
    seen = set()
    unique = []
...
def compute(raw):
    ...
    reasons={k:[] for k in SECTORS}  # tracks why each sector got its score

    def _adj(sector, delta, reason):
        sc[sector] += delta
        reasons[sector].append(reason)

    if cpi>5:
        _adj("Energy",       +2, f"CPI {cpi:.1f}% — commodity inflation drives energy prices")
        _adj("Materials",    +2, f"CPI {cpi:.1f}% — hard assets outperform in high inflation")
        _adj("Real_Estate",  -2, f"CPI {cpi:.1f}% — high inflation erodes real yields, hurts REITs")
        _adj("Technology",   -1, f"CPI {cpi:.1f}% — long-duration growth stocks devalued by inflation")
        _adj("Utilities",    -1, f"CPI {cpi:.1f}% — regulated utilities can't pass costs to consumers")
    elif cpi>3:
        _adj("Energy",       +1, f"CPI {cpi:.1f}% — above-target inflation favours commodity sectors")
        _adj("Materials",    +1, f"CPI {cpi:.1f}% — Materials act as inflation hedge")
        _adj("Real_Estate",  -1, f"CPI {cpi:.1f}% — elevated inflation pressures REIT valuations")

    if fed>5:
        _adj("Financials",   +2, f"Fed rate {fed:.2f}% — wide net interest margin boosts banks")
        _adj("Utilities",    -2, f"Fed rate {fed:.2f}% — high rates make utility dividends uncompetitive")
        _adj("Real_Estate",  -2, f"Fed rate {fed:.2f}% — mortgage costs surge, REIT cap rates compress")
        _adj("Consumer_Disc",-1, f"Fed rate {fed:.2f}% — consumer credit more expensive, spending hurt")
    elif fed>3:
        _adj("Financials",   +1, f"Fed rate {fed:.2f}% — higher rates support bank net interest margins")
        _adj("Utilities",    -1, f"Fed rate {fed:.2f}% — elevated rates pressure yield-sensitive utilities")
        _adj("Real_Estate",  -1, f"Fed rate {fed:.2f}% — tighter mortgage conditions weigh on REITs")
    elif fed<2:
        _adj("Real_Estate",  +1, f"Fed rate {fed:.2f}% — low rates reduce borrowing costs for REITs")
        _adj("Utilities",    +1, f"Fed rate {fed:.2f}% — low rates make utility dividends attractive")
        _adj("Technology",   +1, f"Fed rate {fed:.2f}% — low discount rates boost long-duration growth stocks")

    if cur<-0.5 or cur3m<-0.5:
        _adj("Healthcare",   +2, f"Yield curve inverted ({cur:.2f}%) — recession signal, defensives outperform")
        _adj("Consumer_Stap",+2, f"Yield curve inverted ({cur:.2f}%) — non-discretionary spending holds up in downturns")
        _adj("Utilities",    +1, f"Yield curve inverted ({cur:.2f}%) — defensive dividend yield sought in risk-off")
        _adj("Industrials",  -1, f"Yield curve inverted ({cur:.2f}%) — cyclical capex slows ahead of recession")
        _adj("Consumer_Disc",-2, f"Yield curve inverted ({cur:.2f}%) — discretionary spending first to fall in recessions")
        _adj("Technology",   -1, f"Yield curve inverted ({cur:.2f}%) — risk-off reduces appetite for growth")
    elif cur>1.5:
        _adj("Financials",   +1, f"Yield curve steep ({cur:.2f}%) — banks profit from borrowing short, lending long")
        _adj("Industrials",  +1, f"Yield curve steep ({cur:.2f}%) — signals strong growth expectations ahead")

    if use_gdp<0:
        _adj("Healthcare",   +2, f"GDP {use_gdp:.1f}% — contracting economy, healthcare demand is inelastic")
        _adj("Consumer_Stap",+2, f"GDP {use_gdp:.1f}% — staples spending resilient regardless of growth")
        _adj("Utilities",    +1, f"GDP {use_gdp:.1f}% — utility demand stable even in recessions")
        _adj("Technology",   -2, f"GDP {use_gdp:.1f}% — enterprise IT budgets cut sharply in recessions")
        _adj("Consumer_Disc",-2, f"GDP {use_gdp:.1f}% — discretionary spending collapses when growth contracts")
    elif use_gdp>3:
        _adj("Technology",   +1, f"GDP {use_gdp:.1f}% — strong growth supports IT spend and innovation investment")
        _adj("Industrials",  +1, f"GDP {use_gdp:.1f}% — strong growth drives capex and manufacturing output")
        _adj("Consumer_Disc",+1, f"GDP {use_gdp:.1f}% — robust economy boosts discretionary spending")

    if unr<4:
        _adj("Consumer_Disc",+1, f"Unemployment {unr:.1f}% — tight labour market boosts consumer confidence & spending")
        _adj("Technology",   +1, f"Unemployment {unr:.1f}% — full employment signals broad economic strength")
    elif unr>6:
        _adj("Consumer_Disc",-2, f"Unemployment {unr:.1f}% — rising joblessness crushes discretionary spending")
        _adj("Real_Estate",  -1, f"Unemployment {unr:.1f}% — job losses reduce housing demand")
        _adj("Healthcare",   +1, f"Unemployment {unr:.1f}% — defensive positioning favoured in weak labour market")
        _adj("Consumer_Stap",+1, f"Unemployment {unr:.1f}% — staples outperform when consumers are under stress")

    if m2g>8:
        _adj("Technology",   +1, f"M2 growth {m2g:.1f}% — excess liquidity inflates growth asset multiples")
        _adj("Real_Estate",  +1, f"M2 growth {m2g:.1f}% — money supply expansion historically inflates real assets")
    elif m2g<0:
        _adj("Technology",   -1, f"M2 growth {m2g:.1f}% — shrinking money supply tightens financial conditions")
        _adj("Real_Estate",  -1, f"M2 growth {m2g:.1f}% — liquidity contraction weighs on real asset valuations")

    if retg and retg>5:
        _adj("Consumer_Disc",+1, f"Retail sales +{retg:.1f}% — strong consumer spending supports discretionary names")
    elif retg and retg<0:
        _adj("Consumer_Disc",-1, f"Retail sales {retg:.1f}% — declining consumer spending hurts discretionary sector")

    if vix>30:
        _adj("Healthcare",   +1, f"VIX {vix:.1f} — elevated fear drives flight to defensive healthcare")
        _adj("Consumer_Stap",+1, f"VIX {vix:.1f} — high volatility means investors seek non-cyclical staples")
        _adj("Technology",   -1, f"VIX {vix:.1f} — high VIX suppresses risk appetite for growth names")
        _adj("Consumer_Disc",-1, f"VIX {vix:.1f} — fear environment reduces appetite for cyclical risk")

    if hy>6:
        _adj("Healthcare",   +1, f"HY spread {hy:.2f}% — wide spreads signal credit stress, defensives preferred")
        _adj("Consumer_Stap",+1, f"HY spread {hy:.2f}% — credit stress favours non-cyclical sectors")
        _adj("Financials",   -1, f"HY spread {hy:.2f}% — wide spreads signal rising default risk for lenders")
        _adj("Real_Estate",  -1, f"HY spread {hy:.2f}% — credit tightening hurts leveraged real estate")

    if cfnai<-0.7:
        _adj("Industrials",  -1, f"CFNAI {cfnai:.2f} — broad activity well below trend, capex spending falls")
        _adj("Materials",    -1, f"CFNAI {cfnai:.2f} — below-trend economy reduces raw material demand")
        _adj("Consumer_Disc",-1, f"CFNAI {cfnai:.2f} — weak economic activity suppresses discretionary spending")
    elif cfnai>0.5:
        _adj("Industrials",  +1, f"CFNAI {cfnai:.2f} — above-trend activity signals strong industrial demand")
        _adj("Materials",    +1, f"CFNAI {cfnai:.2f} — above-trend growth drives commodity and material demand")

    if ipyoy<0:
        _adj("Industrials",  -1, f"Industrial production {ipyoy:.1f}% — manufacturing contraction weighs on sector")
        _adj("Materials",    -1, f"Industrial production {ipyoy:.1f}% — falling output reduces input demand")
    elif ipyoy>3:
        _adj("Industrials",  +1, f"Industrial production +{ipyoy:.1f}% — strong output growth boosts the sector")
        _adj("Materials",    +1, f"Industrial production +{ipyoy:.1f}% — factory expansion drives materials demand")

    if sahm>=0.5:
        _adj("Healthcare",   +2, f"Sahm {sahm:.2f} — recession indicator triggered, maximum defensive shift")
        _adj("Consumer_Stap",+2, f"Sahm {sahm:.2f} — recession in progress, staples hold revenue in downturns")
        _adj("Technology",   -2, f"Sahm {sahm:.2f} — recession signal triggers sharp de-rating of growth stocks")
        _adj("Consumer_Disc",-2, f"Sahm {sahm:.2f} — recession confirmed, discretionary spending collapses")

    sc={k:max(-2,min(2,v)) for k,v in sc.items()}
    # Trim reasons to top 2 per sector (most impactful = first added at highest delta)
    reasons={k:v[:3] for k,v in reasons.items()}
    conv=min(100,int((abs(cpi-2.5)+abs(fed-3)+abs(unr-4.5)+abs(cur))*12))
    return dict(reg=reg,col=col,desc=desc_,sc=sc,reasons=reasons,conv=conv,
...
with st.sidebar:
    ...
    st.divider()
    st.markdown("### ℹ About")
    st.caption("**Data:** FRED (Federal Reserve) · yfinance\n\n"
...
with st.spinner("Loading live data feeds…"):
    ...
    econ_cal       = load_econ_calendar(FRED_API_KEY)
...
# ── Signal summary cards (top 3 OW + top 3 UW) with reasons ──────────────────
sorted_all = sorted(sig["sc"].items(), key=lambda x: x[1], reverse=True)
top_ow  = [(k, SLABELS[k], v) for k,v in sorted_all if v > 0][:3]
top_uw  = [(k, SLABELS[k], v) for k,v in reversed(sorted_all) if v < 0][:3]

PTXT_SHORT = {2:"Strong OW", 1:"OW", 0:"Neutral", -1:"UW", -2:"Strong UW"}

def _reason_bullets(key, color):
    rs = sig["reasons"].get(key, [])
    if not rs:
        return f'<div style="font-size:.67rem;color:#4a6080;margin-top:4px">No dominant signals active</div>'
    return "".join(
        f'<div style="font-size:.67rem;color:{color};opacity:.8;margin-top:3px;line-height:1.45">· {r}</div>'
        for r in rs[:2]
    )
...
ss_html = (
    ...
    f'<div style="font-family:{MONO};font-size:.65rem;font-weight:700;color:#4ade80;'
    'text-transform:uppercase;letter-spacing:.1em;margin-bottom:10px">▲ Top Overweights — Why</div>' +
    "".join(
        f'<div style="padding:6px 0;border-bottom:1px solid rgba(74,222,128,.12)">'
        f'<div style="display:flex;justify-content:space-between;align-items:center">'
        f'<span style="color:#dde8f5;font-size:.82rem;font-weight:600">{nm}</span>'
        f'<span style="color:#4ade80;font-family:{MONO};font-weight:700;font-size:.72rem">{PTXT_SHORT[v]}</span></div>'
        f'{_reason_bullets(k, "#4ade80")}'
        f'</div>'
        for k,nm,v in top_ow
    ) +
    '</div>'
    '<div style="background:rgba(248,113,113,.08);border:1px solid rgba(248,113,113,.25);'
    'border-radius:8px;padding:14px 16px">'
    f'<div style="font-family:{MONO};font-size:.65rem;font-weight:700;color:#f87171;'
    'text-transform:uppercase;letter-spacing:.1em;margin-bottom:10px">▼ Top Underweights — Why</div>' +
    "".join(
        f'<div style="padding:6px 0;border-bottom:1px solid rgba(248,113,113,.12)">'
        f'<div style="display:flex;justify-content:space-between;align-items:center">'
        f'<span style="color:#dde8f5;font-size:.82rem;font-weight:600">{nm}</span>'
        f'<span style="color:#f87171;font-family:{MONO};font-weight:700;font-size:.72rem">{PTXT_SHORT[v]}</span></div>'
        f'{_reason_bullets(k, "#f87171")}'
        f'</div>'
        for k,nm,v in top_uw
    ) +
    '</div></div>'
)
...
desc("Dates are sourced from the official <strong>2026 Principal Federal Economic Indicators schedule</strong> "
     "published by the White House OMB — the same authoritative document used by BLS, BEA, Federal Reserve, "
     "and Census Bureau. FOMC dates are supplemented via the FRED API. "
     "High-impact releases (CPI, NFP, GDP, FOMC) typically cause sharp intraday moves in equities, bonds, and FX. "
     "Calendar is updated annually from the OMB publication.")
...
if econ_cal:
    # --- Countdown strip: next 3 events ----------------------------------------
    next3 = econ_cal[:3]
    strip_html = '<div style="display:grid;grid-template-columns:repeat(3,1fr);gap:10px;margin-bottom:20px">'
    for ev in next3:
        ic  = IMPACT_CLR.get(ev["impact"], "#94a3b8")
        ibg = IMPACT_BG.get(ev["impact"], "rgba(148,163,184,.07)")
        da  = ev["days_away"]
        if da == 0:
            countdown = '<span style="color:#f87171;font-weight:700">TODAY</span>'
        elif da == 1:
            countdown = '<span style="color:#fb923c;font-weight:700">TOMORROW</span>'
        else:
            countdown = f'<span style="color:#22d3ee;font-weight:700">In {da} days</span>'
        strip_html += (
            f'<div style="background:{ibg};border:1px solid {ic}55;border-radius:10px;'
            f'padding:16px 18px;border-left:3px solid {ic}">'
            f'<div style="font-family:{MONO};font-size:.63rem;font-weight:700;color:{ic};'
            f'text-transform:uppercase;letter-spacing:.1em;margin-bottom:5px">{ev["impact"]} IMPACT</div>'
            f'<div style="font-size:.9rem;font-weight:700;color:#f0f8ff;margin-bottom:6px">{ev["name"]}</div>'
            f'<div style="font-family:{MONO};font-size:.72rem;color:#8ab4d8">'
            f'{ev["date"].strftime("%a, %d %b %Y")}</div>'
            f'<div style="margin-top:6px;font-size:.75rem">{countdown}</div>'
            f'<div style="font-size:.67rem;color:#4a6080;margin-top:4px">{ev["source"]}</div>'
            f'</div>'
        )
    strip_html += '</div>'
    ...
    # --- Full calendar table ----------------------------------------------------
    st.markdown('<div class="chart-section-title">Full Release Schedule — Next 60 Days</div>',
                unsafe_allow_html=True)
    tbl_html = (
        f"<table style='width:100%;border-collapse:collapse;font-family:{MONO}'>"
        "<thead><tr style='border-bottom:2px solid #1f3354'>"
        + "".join(
            f"<th style='padding:9px 10px;color:#8ab4d8;font-size:.68rem;font-weight:700;"
            f"text-transform:uppercase;letter-spacing:.09em;text-align:{align}'>{h}</th>"
            for h, align in [("Date","left"),("Release","left"),("Days Away","center"),("Impact","center"),("Source","left")]
        )
        + "</tr></thead><tbody>"
    )
    for ev in econ_cal:
        ...
        day_str  = "Today" if da == 0 else ("Tomorrow" if da == 1 else f"{da}d")
        day_col  = "#f87171" if da <= 1 else "#22d3ee" if da <= 7 else "#8ab4d8"
        tbl_html += (
            f"<tr style='border-bottom:1px solid rgba(31,51,84,.5)'>"
            f"<td style='padding:9px 10px;color:#dde8f5;font-weight:600'>"
            f"{ev['date'].strftime('%a %d %b')}</td>"
            f"<td style='padding:9px 10px;color:#f0f8ff;font-size:.84rem'>{ev['name']}</td>"
            f"<td style='padding:9px 10px;text-align:center;"
            f"color:{day_col};font-weight:700;font-size:.78rem'>{day_str}</td>"
            f"<td style='padding:9px 10px;text-align:center'>"
            f"<span style='background:{badge_bg};color:{ic};border:1px solid {ic}55;"
            f"border-radius:4px;padding:2px 9px;font-size:.66rem;font-weight:700;"
            f"text-transform:uppercase;letter-spacing:.08em'>{ev['impact']}</span></td>"
            f"<td style='padding:9px 10px;color:#6a8db0;font-size:.75rem'>{ev['source']}</td>"
            f"</tr>"
        )
    tbl_html += "</tbody></table>"
    st.markdown(tbl_html, unsafe_allow_html=True)
    st.caption(
        f"Source: FRED API (api.stlouisfed.org) · Dates marked '(est.)' are projected from last observation + "
        f"typical release cadence · Refreshed every 6 hours · "
        f"Last fetch: {now_sgt().strftime('%d %b %Y %H:%M')} SGT"
    )
else:
    st.info(
        "Economic calendar could not be loaded. "
        "The calendar queries the FRED API for upcoming scheduled release dates — "
        "this can fail if the FRED API is temporarily rate-limiting or unreachable. "
        "Click **⟳ Refresh** in the sidebar to retry. "
        "If the issue persists, verify your FRED API key at "
...
with hm_col:
    ...
    for k,v in sig["sc"].items():
        reason_lines = sig["reasons"].get(k, [])
        reason_html  = "".join(
            f'<div style="font-size:.64rem;color:#a0bcd8;margin-top:3px;line-height:1.4">· {r}</div>'
            for r in reason_lines
        ) if reason_lines else f'<div style="font-size:.64rem;color:#4a6080;margin-top:3px">No dominant signals</div>'
        cells += (
            f'<div class="hm-cell" style="background:{HMBG[v]};border:1px solid {HMCL[v]}40;'
            f'text-align:left;padding:10px 11px">'
            f'<div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:4px">'
            f'<div class="hm-name" style="color:{HMCL[v]};font-size:.74rem">{SLABELS[k]}</div>'
            f'<div class="hm-sig" style="color:{HMCL[v]};font-size:.68rem;font-weight:700">{PTXT[v]}</div>'
            f'</div>'
            f'{reason_html}'
            f'</div>'
        )
...
st.markdown(
    ...
    f'<span style="font-family:{MONO};font-size:.7rem;color:#4a6080">MACRO/SIGNAL v9.0 · {sgt_f.year}</span>'
    f'</div>',unsafe_allow_html=True)
