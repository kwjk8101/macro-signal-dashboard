"""
MACRO/SIGNAL Dashboard v3.0
============================================================
1. Install dependencies:
   pip install streamlit pandas plotly fredapi yfinance

2. Get a free FRED API key (takes 1 minute):
   https://fred.stlouisfed.org/docs/api/api_key.html

3. Paste your key into FRED_API_KEY below (line 18)
...
.sc-row {
    display: grid;
    grid-template-columns: 120px 68px 22px 1fr;
    gap: 5px;
    align-items: center;
    padding: 5px 0;
...
def apply_theme(fig, height=340):
    fig.update_layout(
        ...
        margin=dict(l=8, r=12, t=32, b=28),
        xaxis=dict(
...
def load_fred_data(api_key):
    ...
    for name, series_id in FRED_IDS.items():
        try:
            raw[name] = fred.get_series(series_id, start, end)
        except Exception:
            pass
    df = pd.DataFrame(raw)
    if "cpi"     in df.columns: df["cpi_yoy"]      = df["cpi"].pct_change(12) * 100
    if "core_cpi"in df.columns: df["core_cpi_yoy"] = df["core_cpi"].pct_change(12) * 100
    if "pce"     in df.columns: df["pce_yoy"]       = df["pce"].pct_change(12) * 100
    if "gdp"     in df.columns: df["gdp_g"]          = df["gdp"].pct_change(4) * 100
    if "m2"      in df.columns: df["m2_g"]           = df["m2"].pct_change(12) * 100
    if "retail"  in df.columns: df["retail_g"]       = df["retail"].pct_change(12) * 100
    ...
    for key, ticker in SECTORS.items():
        try:
            ...
            def pct_change(n):
                ...
                return round((now / closes.iloc[idx] - 1) * 100, 2)
            year_start = closes[closes.index >= f"{datetime.today().year}-01-01"]
            ytd = round((now / year_start.iloc[0] - 1) * 100, 2) if not year_start.empty else None
            rows.append({
                "key":    key,
...
def compute_signals(df):
    ...
    if cpi > 5 and fed > 4:
        ...
        color  = "#f87171"
        desc   = "Fed in active tightening. Risk assets under pressure. Energy, Materials, Financials tend to outperform."
    elif cpi > 3 and fed < 3:
        regime = "INFLATION RESURGENCE · POLICY LAG"
        color  = "#fb923c"
        desc   = "Inflation above target but policy behind the curve. Watch commodities, TIPS, Materials."
        ...
        color  = "#f87171"
        desc   = "Growth contracting. Defensive positioning key. Healthcare, Staples, Utilities, cash preservation."
    else:
...
# ─── Helpers ──────────────────────────────────────────────────────────────────
PILL_CLASS = {2: "sow", 1: "ow", 0: "n", -1: "uw", -2: "suw"}
PILL_TEXT  = {2: "STRONG OW", 1: "OVERWEIGHT", 0: "NEUTRAL", -1: "UNDERWEIGHT", -2: "STRONG UW"}
BAR_COLOR  = {2: "#4ade80", 1: "#86efac", 0: "#475569", -1: "#fca5a5", -2: "#f87171"}
...
def pill_html(score):
    return f'<span class="sig-pill {PILL_CLASS[score]}">{PILL_TEXT[score]}</span>'
...
def pct_html(val):
    ...
    sign  = "+" if val > 0 else ""
    return f'<span style="color:{color};font-family:{MONO_FONT};font-size:0.78rem">{sign}{val:.1f}%</span>'
...
if FRED_API_KEY == "YOUR_FRED_API_KEY_HERE":
    ...
    <div style="text-align:center;padding:4rem 2rem">
      ...
      <div style="font-family:'IBM Plex Mono',monospace;font-size:0.82rem;color:#3a5a7a">
        Paste your FRED API key into the sidebar ←<br>
...
# ─── HEADER ───────────────────────────────────────────────────────────────────
col_title, col_conv, col_count = st.columns([3, 1, 1])
with col_title:
    st.markdown("# 📡 MACRO/SIGNAL")
    st.markdown(
        f'<span class="live-chip">'
        f'<span class="live-dot"></span>'
        f'LIVE &nbsp;·&nbsp; {datetime.now().strftime("%d %b %Y %H:%M")} SGT'
        f'</span>',
        unsafe_allow_html=True,
    )
with col_conv:
    st.metric(
        "Signal Conviction", f"{sig['conviction']}%",
...
kpi_defs = [
    (k1, "Fed Funds Rate",   "fed_rate",     "fed",  ""),
    (k2, "CPI YoY",          "cpi_yoy",      "cpi",  ""),
    (k3, "Core CPI YoY",     "core_cpi_yoy", "core", ""),
    (k4, "Unemployment",     "unrate",       "unr",  "inverse"),
    (k5, "Yield Curve 10-2Y","curve_10_2",   "cur",  ""),
    (k6, "GDP Growth YoY",   "gdp_g",        "gdpg", ""),
]
for col, label, col_key, sig_key, delta_color in kpi_defs:
    v = sig.get(sig_key)
    p = get_prev(df, col_key)
    d = f"{v - p:+.2f}" if v is not None and p is not None else None
    with col:
        st.metric(
            label,
            ...
            help=f"FRED: {FRED_IDS.get(col_key, col_key)}",
        )
...
with left:
    ...
    bar_colors = [BAR_COLOR[v] for v in bar_scores]
    bar_labels = [PILL_TEXT[v] for v in bar_scores]

    fig_bar = go.Figure(go.Bar(
        x=bar_scores, y=bar_names,
        orientation="h",
    ...
    apply_theme(fig_bar, h=380).update_layout(
        xaxis=dict(
            range=[-3.2, 3.2],
            tickvals=[-2, -1, 0, 1, 2],
...
with right:
    ...
    if etfs:
        tbl = (
            ...
            "font-family:IBM Plex Mono,monospace' "
            "role='table' aria-label='Sector ETF live performance and rotation signals'>"
...
with tab2:
    ...
    fig2.update_annotations(font=dict(size=9, color=TICK_CLR))
    st.plotly_chart(fig2, use_container_width=True)
    st.caption("⚠ An inverted yield curve has preceded every US recession since 1955, typically by 6–18 months.")
...
with tab3:
    ...
    fig3.update_annotations(font=dict(size=9, color=TICK_CLR))
...
with tab4:
    ...
    fig4.update_annotations(font=dict(size=9, color=TICK_CLR))
    st.plotly_chart(fig4, use_container_width=True)
    st.caption("M2 contraction (negative YoY) is rare and has historically coincided with stress in credit markets and risk assets.")
...
with hm_col:
    ...
    for k, label in SECTOR_LABELS.items():
        ...
        cells_html += (
            f'<div class="hm-cell" '
            f'style="background:{HM_BG[v]};border:1px solid {HM_CLR[v]}28">'
            f'<div class="hm-name" style="color:{HM_CLR[v]}">{label}</div>'
            f'<div class="hm-sig" style="color:{HM_CLR[v]}">{PILL_TEXT[v]}</div>'
            f'</div>'
        )
    st.markdown(f'<div class="hm-grid">{cells_html}</div>', unsafe_allow_html=True)
...
with sc_col:
    ...
    st.markdown(f'<div class="scorecard-wrap">{rows_html}</div>', unsafe_allow_html=True)
...
st.markdown(
    ...
    <div style="display:flex;justify-content:space-between;padding-top:10px;
      ...
      <span style="font-family:'IBM Plex Mono',monospace;font-size:0.68rem;color:#1e3552">
        Data: Federal Reserve Economic Data (FRED) · yfinance
      </span>
      <span style="font-family:'IBM Plex Mono',monospace;font-size:0.68rem;color:#1e3552">
        ⚠ Educational only — not financial advice
      </span>
      <span style="font-family:'IBM Plex Mono',monospace;font-size:0.68rem;color:#1e3552">
        MACRO/SIGNAL v3.0 · {datetime.now().year}
      </span>
    </div>
    """,
    unsafe_allow_html=True,
