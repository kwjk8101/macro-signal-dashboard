...
def load_econ_calendar(api_key):
    ...
    Strategy 1 (primary): FRED /fred/releases/dates endpoint with
      include_release_dates_with_no_data=true — returns ALL scheduled future
      release dates across every series, then we filter to our target release IDs.

    Strategy 2 (fallback): For each tracked series, pull the last observation
      date via fredapi, then project the next expected release using each
      release's known cadence (monthly, quarterly, weekly, etc.).
    ...
    # FRED release IDs → (display_name, impact_level, source_agency)
    RELEASES = {
        10:  ("CPI Report",                       "HIGH",   "BLS"),
        21:  ("GDP Advance / Preliminary / Final", "HIGH",   "BEA"),
        50:  ("Personal Income & PCE Inflation",   "HIGH",   "BEA"),
        175: ("Non-Farm Payrolls (Employment)",    "HIGH",   "BLS"),
        ...
        11:  ("Consumer Credit",                   "LOW",    "Federal Reserve"),
        82:  ("Initial Jobless Claims",            "MEDIUM", "DOL"),
        148: ("Consumer Confidence",               "MEDIUM", "Conference Board"),
        ...
        180: ("Existing Home Sales",               "LOW",    "NAR"),
    }
    ...
    # ── Strategy 1: /fred/releases/dates (plural) ──────────────────────────────
    # This endpoint returns upcoming scheduled release dates for ALL releases.
    ...
    try:
        ...
        if r.status_code == 200:
            ...
            # Each entry: {"release_id": int, "release_name": str, "date": "YYYY-MM-DD"}
            seen_ids = set()
            for entry in release_dates:
                rid = int(entry.get("release_id", 0))
                if rid not in RELEASES or rid in seen_ids:
                    continue
                ...
                if rel_date < today:
                    continue
                name, impact, source = RELEASES[rid]
                results.append({
                    "name":      name,
                    "date":      rel_date,
    ...
    # ── Strategy 2: per-release /fred/release/dates (singular) ─────────────────
    # Queries each release ID individually. The singular endpoint returns the
    # dates on which each release was *published*, including future scheduled
    # dates when include_release_dates_with_no_data=true is passed.
    if not strategy1_ok:
        fetched_names = set()
        for rid, (name, impact, source) in RELEASES.items():
            ...
            try:
                ...
                if r.status_code != 200:
                    continue
                data = r.json()
                dates_raw = data.get("release_dates", [])
                for entry in dates_raw:
                    ...
                    break  # next upcoming date only
            except Exception:
                continue

    # ── Strategy 3: fredapi last-observation projection ─────────────────────────
    # If both HTTP strategies returned nothing, derive expected next dates from
    # last known FRED series observation + typical release cadence.
    if not results:
        try:
            ...
            # series_id → (display_name, impact, source, cadence_days)
            SERIES_CADENCE = {
                "CPIAUCSL":     ("CPI Report",                       "HIGH",   "BLS",              30),
                "GDPC1":        ("GDP Advance / Preliminary / Final","HIGH",   "BEA",              90),
                "PCEPI":        ("Personal Income & PCE Inflation",  "HIGH",   "BEA",              30),
                "PAYEMS":       ("Non-Farm Payrolls (Employment)",   "HIGH",   "BLS",              30),
                "PPIACO":       ("Producer Price Index (PPI)",       "MEDIUM", "BLS",              30),
                "RSAFS":        ("Retail Sales",                     "MEDIUM", "Census Bureau",    30),
                "HOUST":        ("Housing Starts",                   "MEDIUM", "Census Bureau",    30),
                "IPMAN":        ("Industrial Production",            "MEDIUM", "Federal Reserve",  30),
                "UNRATE":       ("Unemployment Rate",                "HIGH",   "BLS",              30),
                "DGORDER":      ("Durable Goods Orders",             "MEDIUM", "Census Bureau",    30),
                "EXCTHRNSA":    ("Existing Home Sales",              "LOW",    "NAR",              30),
                "SAHMCURRENT":  ("Sahm Rule / Labour Data",          "MEDIUM", "Federal Reserve",  30),
                "CFNAI":        ("Chicago Fed CFNAI",                "MEDIUM", "Federal Reserve",  30),
                "M2SL":         ("M2 Money Supply",                  "LOW",    "Federal Reserve",  30),
            }

            for sid, (name, impact, source, cadence) in SERIES_CADENCE.items():
                try:
                    info = _fred.get_series_info(sid)
                    last_obs = info.get("observation_end")
                    if last_obs:
                        last_date = pd.to_datetime(last_obs).date()
                        # Typical lag: data released ~2-4 weeks after reference period end
                        # Project next release as last_obs + cadence + ~15 day publication lag
                        next_date = last_date + timedelta(days=cadence + 15)
                        # If projected date already passed, try next cycle
                        if next_date < today:
...
desc("All dates are sourced directly from the <strong>FRED API</strong> (Federal Reserve Economic Data), "
     "which publishes official scheduled release dates from BLS, BEA, Federal Reserve, ISM, and Census Bureau. "
     "Where confirmed future dates are not yet published by FRED, the calendar projects the next expected "
     "release from the last known observation date + typical cadence (marked 'est.'). "
     "High-impact releases typically cause sharp intraday moves in equities, bonds, and FX. "
     "This calendar auto-updates every 6 hours.")
